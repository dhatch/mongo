...
public:
    ...
    Status apply() const { return _mod.apply(); }

    Status log(LogBuilder* logBuilder) const { return _mod.log(logBuilder); }
...
TEST(InPlace, DecimalToDecimal) {
    Document doc(BSON("a" << Decimal128("1")));
    Mod incMod(BSON("$inc" << BSON("a" << Decimal128("1"))));
    ModifierInterface::ExecInfo execInfo;
    ASSERT_OK(incMod.prepare(doc.root(), "", &execInfo));
    ASSERT_FALSE(execInfo.noOp);
}
