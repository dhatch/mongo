...
void keyTest(const BSONObj& o, bool mustBeCompact = false) {
    ...
    if (kLast) {
        ...
        ASSERT(ok);
        if (k.isCompactFormat() && kLast->isCompactFormat()) {  // only check if not bson as bson
                                                                // woEqual is broken! (or was
                                                                // may2011)
...
public:
    void run() { ASSERT(basic("a", 1).woCompare(basic("a", 1.0)) == 0); }
};
...
struct AppendNumber {
    void run() {
        ...
        b.appendNumber("f", mongo::Decimal128("123.54"));
...
protected:
    ...
    static char get(const BSONObj& o, int i) { return o.objdata()[i]; }
    static void set(BSONObj& o, int i, char c) { const_cast<char*>(o.objdata())[i] = c; }
};

class BadType : public Base {
    BSONObj valid() const { return fromjson("{\"a\":1}"); }
...
class WrongStringSize : public Base {
    BSONObj valid() const { return fromjson("{\"a\":\"b\"}"); }
    BSONObj invalid() const {
        BSONObj ret = valid();
...
class WrongSubobjectSize : public Base {
    BSONObj valid() const { return fromjson("{\"a\":{\"b\":1}}"); }
    BSONObj invalid() const {
        BSONObj ret = valid();
        set(ret, 0, get(ret, 0) + 1);
...
class BadRegex : public Base {
    BSONObj valid() const { return fromjson("{\"a\":/c/i}"); }
    BSONObj invalid() const {
        BSONObj ret = valid();
...
public:
    virtual ~LabelBase() {}
    void run() { ASSERT(!expected().woCompare(actual())); }
...
class LabelBasic : public LabelBase {
    BSONObj expected() { return BSON("a" << (BSON("$gt" << 1))); }
    BSONObj actual() { return BSON("a" << GT << 1); }
};
...
class LabelDouble : public LabelBase {
    ...
    BSONObj actual() { return BSON("a" << GT << 1 << LTE << "x"); }
};
...
class LabelSize : public LabelBase {
    BSONObj expected() { return BSON("a" << BSON("$size" << 4)); }
    BSONObj actual() { return BSON("a" << mongo::BSIZE << 4); }
};
...
public:
    BSONObj one(string s) { return BSON("x" << s); }
    ...
    void lt(BSONObj order, BSONObj l, BSONObj r) { test(order, l, r, 1); }
...
public:
    void good(string s) { good(fromjson(s)); }
    ...
    void bad(string s) { bad(fromjson(s)); }
